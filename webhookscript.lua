...
local function sendWebhook(name, rarity, shiny, chance)
    ...
    if chance and chance ~= "" and not string.find(chance, " in ", 1, true) then
        return
    end
    ...
    -- Determine variant (normal, shiny, mythic, shiny mythic)
    ...
    -- Description with ðŸ¥š emoji for hatches
    local desc = string.format(
        "ðŸŽ²  __**Chance**__: %s\nðŸ£ __**Hatch Date**__: <t:%d:R>\nðŸ¥š  __**Hatches**__: %s\nðŸ•µï¸  __**Hatched By**__: %s",
        chance or "N/A",
        time,
        totalHatches,
        player.Name
    ...
    if hatch then
        for _, frame in ipairs(hatch:GetChildren()) do
            if frame:IsA("Frame") and frame:FindFirstChild("Label") and frame:FindFirstChild("Rarity") then
                ...
                if (rarityLower:find("legendary") or rarityLower:find("secret"))
